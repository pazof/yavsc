...
namespace Yavsc.Model.Blogs
{
	...
	public static class BlogManager 
	{
		...
		/// <param name="username">Username.</param>
		/// <param name="title">Title.</param>
		public static void RemoveTitle (string username, string author, string title)
		{
			if (!Roles.IsUserInRole ("Admin")) {
				string rguser = username;
				if (rguser != author) {
			...
			Provider.RemoveTitle (author, title);
		}
		/// <summary>
		/// Gets the tag info.
		/// </summary>
		...
		/// <param name="pageSize">Page size.</param>
		/// <param name="totalRecords">Total records.</param>
		public static IEnumerable<BlogEntry> LastPosts (string userName, int pageIndex, int pageSize, out int totalRecords)
		{
			var c =  Provider.LastPosts (pageIndex, pageSize, out totalRecords);
			return FilterOnReadAccess (userName,c);
		}
		...
		public static BlogEntry GetForEditing (string userName, long postid, bool throwEx = true)
		{
		...
		private static bool CanView (BlogEntry e, string userName)
		{
			if (e.AllowedCircles != null && e.AllowedCircles.Length > 0) {
				...
				if (!Roles.IsUserInRole (userName, "Admin"))
				{
					if (!e.Visible)
						return false;
					if (!CircleManager.DefaultProvider.Matches (e.AllowedCircles, userName))
						return false;
				}
			}
			return true;
		...
		/// <param name="postid">Postid.</param>
		/// <param name="throwEx">If set to <c>true</c> throw ex.</param>
		public static BlogEntry GetForReading (string username, long postid, bool throwEx = true)
		{
			BlogEntry e = BlogManager.GetPost (postid);
			if (e == null) 
			if (throwEx)
				throw new FileNotFoundException ();
			if ( CanView (e, username))
				return e;
			if (throwEx)
		...
		/// <param name="throwEx">If set to <c>true</c> throw ex.</param>
		private static bool HasReadAccess (string username, BlogEntryCollection bec, bool throwEx = true)
		{
			if (bec == null)
				throw new FileNotFoundException ();
			if (Roles.IsUserInRole ("Admin"))
				return true;
			BlogEntry e = bec.First (x=>!CanView(x,username));
			if (e == null)
				return true;
			if (throwEx)
				throw new AccessViolationException (
		...
		public static IEnumerable<TEntry> FilterOnReadAccess<TEntry> (string userName, IEnumerable<TEntry> bec)
		{
			if (bec == null) return null;
			if (Roles.IsUserInRole ("Admin")) return bec;
			var r = bec.Where (x => CanView (x as BlogEntry, userName));
			return r;
		}
		/// <summary>
