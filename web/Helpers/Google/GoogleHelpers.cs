...
namespace Yavsc.Helpers
{
	...
	public static class GoogleHelpers
	{
		...
		public static CalendarEventList GetEvents(this ProfileBase profile, DateTime mindate, DateTime maxdate)
		{
			...
			if (gcalid == null)
				throw new InvalidProgramException ("gcalid");
			CalendarApi auth = new CalendarApi ("Google", "/OAuth/ExternalCallback"); 
			string creds = auth.GetCredential(profile.UserName);
			return auth.GetCalendar (gcalid, mindate, maxdate, creds);
		}
		...
		public static CalendarList GetCalendars (this ProfileBase profile)
		{
			CalendarApi auth = new CalendarApi ("Google", "/OAuth/ExternalCallback"); 
			string creds = auth.GetCredential(profile.UserName);
			return auth.GetCalendars (creds);
		}

		static string [] googleLoginScopes = { "openid","profile","email"};
		...
		{
			GoogleAuth oa = new PeopleApi ("Google", callBack);
			oa.Login (response, state,
				oa.Id, googleLoginScopes
			);
		}
		/// <summary>
		/// Cals the login.
		...
		{
			CalendarApi oa = new CalendarApi ("Google", callBack);
		...
		/// <param name="callBack">Call back.</param>
		public static T CreateOAuth2<T>(string callBack = "/OAuth/ExternalCallback", string api = "People")
			where T : GoogleAuth
		{
			switch (api) {
			case "Tracks":
				return new MapTracks ("Google",callBack) as T;
			case "Calendar":
				return new CalendarApi ("Google",callBack) as T;
			case "People":
				return new PeopleApi ("Google",callBack) as T;
			}
			return null;
		}
