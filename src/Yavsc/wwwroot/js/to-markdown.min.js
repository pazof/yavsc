...
return isFlanked}
function flankingWhitespace(node,content){var leading=''
var trailing=''
...
return'\n\n'+hPrefix+' '+content+'\n\n'}},{filter:'hr',replacement:function(){return'\n\n* * *\n\n'}},{filter:['em','i'],replacement:function(content){return'_'+content+'_'}},{filter:['strong','b'],replacement:function(content){return'**'+content+'**'}},{filter:['u'],replacement:function(content){return'_'+content+'_'}},{filter:['del','s','strike'],replacement:function(content){return'~~'+content+'~~'}},{filter:'div',replacement:function(content){return content+'\n\n'}},{filter:function(node){var hasSiblings=node.previousSibling||node.nextSibling
...
return'['+content+']('+node.getAttribute('href')+titlePart+')'}},{filter:'video',replacement:function(content,node){var alt=node.getAttribute('alt')||''
var src
for(var i=0;i<node.childNodes.length;i++){if(node.childNodes[i].localName==='source'){src=node.childNodes[i].getAttribute('src')
break}}
var title=node.title||''
var titlePart=title?' "'+title+'"':''
return src?'![video:'+alt+']'+'('+src+titlePart+')':''}},{filter:'audio',replacement:function(content,node){var alt=node.getAttribute('alt')||''
var src=node.getAttribute('src')||''
if(!src){for(var i=0;i<node.childNodes.length;i++){if(node.childNodes[i].localName==='source'){src=node.childNodes[i].getAttribute('src')
break}}}
var title=node.title||''
var titlePart=title?' "'+title+'"':''
return src?'![audio:'+alt+']'+'('+src+titlePart+')':''}},{filter:'img',replacement:function(content,node){var alt=node.alt||''
var src=node.getAttribute('src')||''
var title=node.title||''
var titlePart=title?' "'+title+'"':''
